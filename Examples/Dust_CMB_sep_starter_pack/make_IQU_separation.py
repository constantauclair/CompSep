...
and noise contamination on Planck data. The aim is to obtain the statistics of the Q and U dust 
emission as well the IQ and IU correlation. It makes use of the WPH statistics (see Régaldo-Saint Blancard et al. 2022). 
For any question: constantauclair1998@gmail.com
Another project has been led on the dust/CIB/noise separation on Herschel data (see Auclair et al. 2024).

The quantities involved are d (the total map), u (the optimized dust map), n (the noise + CMB map) for IQU Stokes parameters. 
This algorithm makes use of full-mission and half-missions (1 and 2) maps, indexed respectively using FM, HM1 and HM2.
A parser is used to choose the frequency (0 for 100, 1 for 143, 2 for 217 or 3 for 353)
  
The algorithm makes use of the following loss terms:
...
The returned file is a [9,M,N] numpy array correponding to :

Index    Map
  0      Raw I
  1      Raw Q 
  2      Separated dust Q
  3      Separated CMB+Noise Q
  4      Intermediate separated dust Q (end of the first step)
  5      Raw U
  6      Separated dust U
  7      Separated CMB+Noise U
  8      Intermediate separated dust U (end of the first step)
...
file_name="IQU_separation_"+str(freqs[freq])+"GHZ.npy"
...
# Dust 
dust_maps = np.load('Sroll20_IQU_maps.npy').astype(np.float32)
...
# CMB
cmb_maps = np.load('CMB_IQU_maps.npy').astype(np.float32)
...
# Noise
noise_Q = np.load('Sroll20_Noise_Q_'+str(freqs[freq])+'_maps.npy').astype(np.float32)
n_Q_FM = noise_Q[0,:Mn]
n_Q_HM1 = noise_Q[1,:Mn]
n_Q_HM2 = noise_Q[2,:Mn]
del noise_Q
noise_U = np.load('Sroll20_Noise_U_'+str(freqs[freq])+'_maps.npy').astype(np.float32)
n_U_FM = noise_U[0,:Mn]
...
# I map
I = np.load('Sroll20_I_857_FM_768px.npy').astype(np.float32)
